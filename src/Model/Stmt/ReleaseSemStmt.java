...
public class ReleaseSemStmt implements IStmt{
    ...
    public PrgState execute(PrgState state) throws MyException {
        ...
        Pair<List<Integer>, Integer> semaphoreList = semaphoreEmtry.getValue();
        Integer currentThreadId = state.getId();
        if(semaphoreList.getKey().contains(currentThreadId))
            semaphoreList.getKey().remove(currentThreadId);
        lock.unlock();
        return null;
    }
